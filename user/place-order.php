...
<body>
    ...
    <nav>
      <ul>
        ...
        <li>
          ...
          ?>
          <i class="bi bi-cart" onclick="document.querySelector('.cart-container').style.right = '0'">
  ...
  <main class="place-Order-container">
    ...
    $buyProduct_ID = $_SESSION['buyProduct_ID'];
    $buyProduct_Quantity = $_SESSION['buyProduct_Quantity'];
    $buyProduct_Size = $_SESSION['buyProduct_Size'];
    $buyProduct_AddOns = $_SESSION['buyProduct_AddOns'];
    $addOn_price = $_SESSION['addOns_price'];

    $sql = "SELECT COUNT(product_ID) as product_count, product_img, product_name, product_price FROM product WHERE product_ID = :buyProduct_ID";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':buyProduct_ID', $buyProduct_ID);
    ...
    if (COUNT($result) > 0) {
      ...
      $product_subtotal = $result['product_price'] * $buyProduct_Quantity;

      $addOn_total = $_SESSION['addOns_price'] * $buyProduct_Quantity;

      $product_total = $product_subtotal + $addOn_price;
    ?>
      ...
      <section class="left">
        ...
        <div class="productOrder-information">
          ...
          <div class="user-product-info">
            <ul>
              <li><img src="../img/products/<?php echo $result['product_img'] ?>" alt="" /></li>
              <li><?php echo $result['product_name'] ?> <br> <span style="font-size: 1.2rem;"><?php echo $buyProduct_Size . ", " . $buyProduct_AddOns; ?> </span> </li>

              <li>₱ <?php echo number_format((float)$result['product_price'], 2, '.', ','); ?></li>
              <li>
                Quantity: <?php echo $buyProduct_Quantity ?>
                <input type="hidden" name="product_quantity" value="<?php echo $buyProduct_Quantity ?>">
              </li>
            </ul>
          </div>
        </div>
        ...
        <section class="right">
          ...
          <div class="order-summary">
            ...
            <div class="order-subtotal">
              <span>Subtotal (<?php echo $productCount ?> Items)</span>
              <label for="order-subtotal">₱ <?php echo number_format((float)$product_subtotal, 2, '.', ','); ?></label>
              <input type="hidden" name="product_subtotal" id="order-subtotal" />
            ...
            <div class="order-subtotal">
              ...
              <span>Add Ons</span>
              <label for="order-subtotal">₱ <?php echo number_format((float)$addOn_total, 2, '.', ','); ?></label>
              <input type="hidden" name="product_subtotal" id="order-subtotal" />
          ...
          <div class="place-order">
            <div class="product-total-order">
              ...
              <label for="user-order-total">₱
                <?php
                echo number_format((float)$product_total, 2, '.', ',');
                $_SESSION['order_total'] = $product_total;
                ?></label>
            </div>
            <input type="hidden" name="user_ID" value="<?php echo $_SESSION['user_ID'] ?>">
            <button name="order_now" onclick="simulateLoading()" style="cursor: pointer;">ORDER NOW</button>
          </div>
        </section>
      </form>
        ...
        <div class="order-container">
          ...
          <?php foreach ($result as $cart): ?>
            <div class="cart-product-container">
              ...
              <ul class="cart-right">
                <!-- ADD ONS PRICE -->
                <li> <?php echo $addOn_Price * $cart['product_quantity'] ?></li>
                <!-- PRODUCT PRICE -->
    ...
    <div class="order-placed" style="display: none;">
      ...
      <div class="redirect-links">
        <a href="userHomepage1.php">Return to homepage</a> <a href="menu5.php">Order again</a>
      </div>
    </div>
